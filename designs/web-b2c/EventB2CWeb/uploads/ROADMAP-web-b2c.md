...
> **Figma file:** [StartupMVP — Web B2C](https://www.figma.com/design/NQsJJ3f1GPTMlq8GSod9Qt/StartupMVP?node-id=1-2)
> **Page:** `Web-b2c`

The Figma file contains the following screens that define the visual direction for the Web B2C application:

| Screen | Figma Node | Description |
|--------|------------|-------------|
| **Main page** | `7:7226` | Split-view layout: scrollable event list (left) + interactive map with category-colored pins (right). Top bar includes logo, city selector dropdown, search input, "For Organizers" link, and language toggle. Horizontal category filter bar with icon chips (Live Now, Music, Club, Art, Food, Sport, Theatre, Comedy, Tech, Film, Festival, Wellness, Kids). Date filter ("Any date") and sort control ("Relevance"). Event cards show thumbnail, category badge, title, date/time, venue, price, and optional tags ("Fast", "Weekly", community scout attribution). Cookie consent banner overlays the map. Footer with Discover links, For Organizers, Legal, Get the App (App Store / Google Play), and social icons. |
| **City picker — no match** | `10:9386` | Modal overlay "Choose your city" with search input. When the searched city is not available, shows "No matching cities found" and a suggestion CTA: "[City] isn't available yet — Let us know you'd like to see events there" with a "Suggest [City]" button. |
| **City picker — city list** | `10:9948` | Modal overlay "Choose your city" with searchable list of supported cities (Poznan, Krakow, Wroclaw, Warszawa, etc.). |
| **Event Details page** | `10:10511` | Full event detail view: hero image with category badge and "Selling Fast" tag, share button overlay. Two-column layout below hero — left: event title, date/time with icon, venue with address, "About this event" description, hashtags (#electronic, #jazz, #live band), "Add to calendar" section (Google Calendar, Apple Calendar .ics, Outlook .ics). Right sidebar: price card (price, urgency indicator, "Buy Tickets" CTA button in violet), Navigate and Save buttons, venue card with avatar and follower count, source attribution. Smart banner: "Save this event in the app — Get reminders and updates on your phone" with App Store / Play Store buttons. "You might also like" section with related event cards. Full footer. |
| **Cookie Banner** | `10:10740` | Compact cookie consent banner: "We use cookies — We use cookies to enhance your experience, analyze site traffic, and personalize content." Three actions: "Accept All" (primary violet), "Reject All" (outline), "Manage" (text). |
| **Venue Profile page** | `10:10798` | Venue detail view: hero image, category badge, venue name, address, follower count. Description text. "Is this your venue? Claim it on the Organizer Dashboard" CTA link. Right sidebar: venue card with "Follow in the app" prompt and App Store / Play Store buttons, embedded map with address and "Navigate" link. "Upcoming events" section with event cards. Full footer. |
| **Date picker — Today** | `11:11042` | Modal "Choose dates" with quick-select preset chips: Today (active/selected), Tomorrow, This weekend, This week. "Or pick a range" section with month calendar (May 2026), today highlighted. Clear and Apply (violet) buttons. |
| **Date picker — This week** | `11:11696` | Same date picker modal with "This week" preset selected, showing the current week range highlighted on the calendar. |
...
- [ ] P0 **First-visit default state:**
  ...
  - Cookie consent banner fires immediately (per section 1.10)
